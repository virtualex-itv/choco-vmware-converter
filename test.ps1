...
if ($env:APPVEYOR_BUILD_VERSION) {
  ...
  # run manually
  [xml]$spec = Get-Content vmware-converter.nuspec
  $version = $spec.package.metadata.version
}

"TEST: Version $version in vmware-converter.nuspec file should match"
[xml]$spec = Get-Content vmware-converter.nuspec
...
$zip = [IO.Compression.ZipFile]::OpenRead("$pwd\vmware-converter.$version.nupkg")
...
. choco install -y vmware-converter -source . -version $version

"TEST: Uninstallation of package should work"
. choco uninstall -y vmware-converter -source .
